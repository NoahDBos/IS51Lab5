...
then from the text file, we create a list of string from each line in the text file.
Then, we create a dictionary the list.
once the text file has been read into memory, we close the file.

We then define a function for translating which envolces splitting the user input (sentence) into an
array of strings ("enjoy the excellent band tonight") ["enjoy", "the", "excellent", "band", "tonight"]

once we have the array of strings representing the user's input sentence, we loop
through each words, find the key matching the word and returns back the value tied to that word
in our dictionary.
...
translate(sentence, dictionary):
    ...
    print translated sentene to user
